...
async function handleFFXIVCommand(message, client, member) {
  ...
  if (!role) {
    ...
    message.reply(
      `The role 'FFXIV' does not exist. Please contact an administrator.`
    );
    return;
  }
  ...
  try {
    ...
    if (botMember.permissions.has(PermissionsBitField.Flags.ManageRoles)) {
      ...
      if (botMember.roles.highest.comparePositionTo(role) > 0) {
        ...
        message.reply(
          `OK ${member.displayName}, I have given you the FFXIV role.`
        );
      } else {
        // Reply to the user if the bot's role is not high enough
        message.reply(
          `Sorry ${member.displayName}, my role is not high enough to assign the FFXIV role.`
        );
      }
    } else {
