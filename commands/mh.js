...
async function handleMHCommand(message, client, member) {
  ...
  const role = message.guild.roles.cache.find(
    (role) => role.name === "Monster Hunter"
  );
  if (!role) {
    // Reply to the user if the "Monster Hunter" role does not exist
    message.reply(
      `The role 'Monster Hunter' does not exist. Please contact an administrator.`
    );
    return;
  }
  ...
  try {
    ...
    if (botMember.permissions.has(PermissionsBitField.Flags.ManageRoles)) {
      ...
      if (botMember.roles.highest.comparePositionTo(role) > 0) {
        ...
        message.reply(
          `OK ${member.displayName}, I have given you the MH role.`
        );
      } else {
        // Reply to the user if the bot's role is not high enough
        message.reply(
          `Sorry ${member.displayName}, my role is not high enough to assign the MH role.`
        );
      }
    } else {
      // Reply to the user if the bot does not have permission to manage roles
      message.reply(
        `Sorry ${member.displayName}, I don't have permission to manage roles.`
      );
    }
  } catch (error) {
